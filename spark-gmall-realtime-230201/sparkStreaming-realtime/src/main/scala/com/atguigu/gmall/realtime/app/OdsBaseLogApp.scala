              ...
              if (pageObj != null) {
                ...
                if (actionJsonArr != null && actionJsonArr.size() > 0) {
                  for (i <- 0 until actionJsonArr.size()) {
                    ...
                    val pageActionLog = PageActionLog(mid, uid, ar, ch, isNew, md, os, vc, ba, pageId, lastPageId, pageItem, pageItemType, sourceType, duringTime, actionId, action_item, action_item_type, ts)
